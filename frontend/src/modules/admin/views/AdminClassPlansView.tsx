...
import { useAdminClassPlans } from '../hooks/useAdminClassPlans';
import { CreateClassPlanModal, EditClassPlanModal } from '../components/AdminClassPlanModals';

const MONTHS_ES = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];

export function AdminClassPlansView() {
  const {
    classPlans, total, loading, error, classGroupFilter, setClassGroupFilter, monthFilter, setMonthFilter, yearFilter, setYearFilter,
    page, setPage, pageSize, classGroups,
    ...
    <div className="min-h-screen bg-background text-text">
      ...
      <main className="w-full max-w-md sm:max-w-lg md:max-w-2xl mx-auto p-4 pb-24 space-y-4">
        <div className="bg-surface rounded-2xl border border-border shadow-soft p-4 flex items-center justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-primary font-bold">Total configuraciones</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="sm:col-span-3">
                <p className="text-xs uppercase tracking-[0.2em] text-primary font-bold">Comisión</p>
                <select className="mt-2 w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm" value={classGroupFilter} onChange={(e) => setClassGroupFilter(e.target.value)}>
                  <option value="">Todas las comisiones</option>
                  {classGroups.map((cg) => <option key={cg.id} value={cg.id}>{cg.name || `Comisión ${cg.id}`}</option>)}
                </select>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-primary font-bold">Mes</p>
                <select className="mt-2 w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm" value={monthFilter} onChange={(e) => setMonthFilter(e.target.value)}>
              ...
              <div>
                ...
                <select className="mt-2 w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm" value={yearFilter} onChange={(e) => setYearFilter(e.target.value)}>
                  <option value="">Todos</option>
        ...
        {!loading && !error && classPlans.length === 0 && <div className="bg-surface p-4 rounded-xl text-sm text-muted border border-border">No hay planes de clases para mostrar.</div>}
