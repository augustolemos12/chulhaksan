...
import { useAdminClassGroups, type DayOfWeek } from '../hooks/useAdminClassGroups';
import { CreateClassGroupModal, EditClassGroupModal } from '../components/AdminClassGroupModals';
...
export function AdminClassGroupsView() {
  const {
    classGroups, total, loading, error, gymFilter, setGymFilter, teacherFilter, setTeacherFilter,
    page, setPage, pageSize, gyms, teachers,
  ...
  return (
    <div className="min-h-screen bg-background text-text">
      ...
      <main className="w-full max-w-md sm:max-w-lg md:max-w-2xl mx-auto p-4 pb-24 space-y-4">
        ...
        {!loading && !error && classGroups.length === 0 && <div className="bg-surface p-4 rounded-xl text-sm text-muted border border-border">No hay comisiones para mostrar.</div>}

        <div className="flex flex-col gap-3">
          {classGroups.map((cg) => (
            <div key={cg.id} className="flex flex-col sm:flex-row sm:items-center gap-4 bg-surface p-4 rounded-xl justify-between shadow-soft">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted mt-1 truncate">
                  Prof: <span className="font-medium text-text">{cg.teacher?.firstName} {cg.teacher?.lastName}</span>
                </p>
                <p className="text-xs text-muted mt-1 truncate">
              ...
              <div className="flex sm:flex-col gap-2 shrink-0">
                <button className="flex-1 rounded-lg bg-primary text-white text-xs font-semibold px-3 py-2" type="button" onClick={() => openEdit(cg)}>Editar</button>
                <button className="flex-1 rounded-lg border border-red-200 text-red-600 text-xs font-semibold px-3 py-2" type="button" onClick={() => handleDelete(cg)}>Eliminar</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <CreateClassGroupModal
        ...
        gyms={gyms} teachers={teachers} handleCreate={handleCreate} creating={creating} createError={createError}
      />
      
      <EditClassGroupModal
        editing={editing} setEditing={setEditing} form={form} setForm={setForm}
        gyms={gyms} teachers={teachers} handleSave={handleSave} saving={saving} editError={editError}
      />
    </div>
  );
}
