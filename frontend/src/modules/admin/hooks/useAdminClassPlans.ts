...
export function useAdminClassPlans() {
  ...
  const loadDependencies = async () => {
    try {
      const res = await httpClient.request('/class-groups?limit=100', { cache: 'no-store' });
      if (res.ok) {
  ...
  useEffect(() => { loadClassPlans(); }, [page, classGroupFilter, monthFilter, yearFilter]);
  useEffect(() => { setPage(1); }, [classGroupFilter, monthFilter, yearFilter]);
  ...
  }, [classGroupFilter, monthFilter, yearFilter, page]);
  ...
  return {
    ...
    page, setPage, pageSize, classGroups, searchParams, setSearchParams,
