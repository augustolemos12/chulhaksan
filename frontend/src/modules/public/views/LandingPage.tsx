...
const SOCIAL_LINKS = [
  { name: 'WhatsApp', href: 'https://wa.me/5492612793740', icon: 'chat' },
  { name: 'Facebook', href: 'https://www.facebook.com/share/1DhjYkWfU9/?mibextid=wwXIfr', icon: 'thumb_up' },
  { name: 'Instagram', href: 'https://www.instagram.com/chulhaksan.mendoza', icon: 'photo_camera' },
];

const TENETS = [
  { label: 'Cortesía', korean: 'Ye Ui', icon: 'front_hand', isPrimary: true },
  { label: 'Integridad', korean: 'Yom Chi', icon: 'verified_user', isPrimary: false },
  { label: 'Perseverancia', korean: 'In Nae', icon: 'speed', isPrimary: false },
  { label: 'Autocontrol', korean: 'Guk Gi', icon: 'psychology', isPrimary: false },
  { label: 'Espíritu Indomable', korean: 'Baekjul Boolgool', icon: 'bolt', isPrimary: true, colSpan: 2 },
];
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center bg-surface/95 backdrop-blur-md px-4 py-3 justify-between border-b border-border">
      <div className="flex items-center gap-3">
        <img alt="Chul Hak San" className="h-9 w-9 rounded-full bg-surface p-0.5 shadow-soft" src={logoColor} />
    ...
    <section className="relative py-12 px-4 bg-background overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-md sm:max-w-lg md:max-w-2xl mx-auto relative z-10">
        {TENETS.map((tenet, idx) => (
          <div
            key={idx}
            className={`${tenet.colSpan ? 'col-span-2' : ''} bg-surface p-5 rounded-2xl border ${tenet.isPrimary ? 'border-border' : 'border-primary/40'} shadow-soft transition-all duration-300 flex flex-col items-center text-center`}
          >
            <div className={`w-12 h-12 rounded-full ${tenet.isPrimary ? 'bg-primary/10' : 'bg-primary/5'} flex items-center justify-center mb-3`}>
