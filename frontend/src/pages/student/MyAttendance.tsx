        ...
        <div className="min-h-screen bg-background">
            <header className="sticky top-0 z-20 bg-background/80 backdrop-blur-md border-b border-border">
                <div className="w-full max-w-md sm:max-w-lg md:max-w-2xl mx-auto p-4 flex items-center justify-between">
                    <button
                        className="flex size-10 items-center justify-center rounded-xl hover:bg-surface transition-colors"
                        type="button"
                        onClick={() => navigate(-1)}
                        aria-label="Volver"
                    >
            ...
            <main className="w-full max-w-md sm:max-w-lg md:max-w-2xl mx-auto p-4 pb-24 space-y-5">
                <section className="bg-surface rounded-3xl border border-border shadow-soft p-5">
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <h2 className="text-xl font-bold mt-1 text-text">
                ...
                {loading && (
                    <div className="bg-surface p-5 rounded-2xl text-sm text-muted border border-border shadow-soft">
                        Cargando asistencias...
                    </div>
                )}

                {error && (
                    <div className="bg-red-50 p-5 rounded-2xl text-sm text-red-600 border border-red-200 shadow-soft">
                        {error}
                    </div>
                )}
                ...
                {!loading && !error && rows.length > 0 && (
                    <section className="space-y-3">
                        ...
                        {rows.map((r) => (
                            <div
                                key={r.id}
                                className="bg-surface rounded-2xl border border-border shadow-soft p-4 flex items-center justify-between gap-4 hover:shadow-md transition-all"
                            >
