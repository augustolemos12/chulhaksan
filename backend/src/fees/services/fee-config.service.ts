  ...
  async createFeeConfig(data: CreateFeeConfigDto) {
    if (
      ...
      throw new BadRequestException(
        'El monto base de la cuota debe ser mayor a 0.',
      );
    }
    ...
    if (latestConfig && newValidFrom <= latestConfig.validFrom) {
      throw new BadRequestException(
        'La fecha de vigencia debe ser posterior a la última configuración existente.',
      );
    }
  ...
  async getLatestFeeConfig() {
    ...
    if (!config) {
      throw new NotFoundException(
        'No active FeeConfig found. Please create one.',
      );
    }
